...
class Handler:
    ...
    def handle_expr_context(
        ...
        if isinstance(ctx, stellaParser.VarContext):
            ...
            self.handle_expr_context(
                ctx=ctx.args[0],
                ctx_type=function_type['return_type'].paramTypes[0],
                variables=variables,
            )

            assert repr_type(ctx_type) == repr_type(function_type['return_type'])
    ...
    def handle_decl_context(
        ...
        if isinstance(ctx, stellaParser.DeclFunContext):
            ...
            function_type = stellaParser.TypeFunContext(
                ctx=ctx,
                parser=ctx.parser,
            )

            function_type.returnType = ctx.returnType
            function_type.paramTypes = [ctx.paramDecls[0].paramType]
            ...
            for parameter in ctx.paramDecls:
                variables.update({
                    parameter.name.text: {'return_type': parameter.paramType},
                })
